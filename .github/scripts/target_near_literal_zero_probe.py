...
def transform_cpi_text_relocs(src, dst, symbols, mode, target_hash, target_offset_hex, log_path):
    ...
    mapval_match = re.fullmatch(
        r"mapval_([0-9]+)_((?:[0-9]+to[0-9]+)(?:_[0-9]+to[0-9]+)*)_((?:v[0-9]+eq[0-9]+)(?:_v[0-9]+eq[0-9]+)*)",
        mode,
    )
    mapname_match = re.fullmatch(
        r"mapname_([0-9]+)_((?:[0-9]+to[0-9]+)(?:_[0-9]+to[0-9]+)*)_((?:n[0-9]+eq[0-9]+)(?:_n[0-9]+eq[0-9]+)*)",
        mode,
    )
    mapentry_match = re.fullmatch(
        r"mapentry_([0-9]+)_((?:[0-9]+to[0-9]+)(?:_[0-9]+to[0-9]+)*)_((?:e[0-9]+eq[0-9]+)(?:_e[0-9]+eq[0-9]+)*)",
        mode,
    )
    mapdelta_match = re.fullmatch(
        r"mapdelta_([0-9]+)_((?:[0-9]+to[0-9]+)(?:_[0-9]+to[0-9]+)*)_((?:d[0-9]+[pm][0-9a-fA-F]+)(?:_d[0-9]+[pm][0-9a-fA-F]+)*)",
        mode,
    )
    ...
    addrpatchadrp_match = re.fullmatch(
        r"addrpatchadrp_([0-9]+)_([0-9]+)_at([0-9a-fA-F]+)(?:_((?:d[0-9]+[pm][0-9a-fA-F]+)(?:_d[0-9]+[pm][0-9a-fA-F]+)*))?",
        mode,
    )
    ...
    addrpatchpairregs_match = re.fullmatch(
        r"addrpatchpairregs_([0-9]+)_adrp([0-9]+)_after([0-9]+)_at([0-9a-fA-F]+)_aw([0-9a-fA-F]+)_lw([0-9a-fA-F]+)(?:_((?:d[0-9]+[pm][0-9a-fA-F]+)(?:_d[0-9]+[pm][0-9a-fA-F]+)*))?",
        mode,
    )
    targetword_match = re.fullmatch(
        r"targetword_([0-9]+)_w([0-9a-fA-F]+)(?:_((?:d[0-9]+[pm][0-9a-fA-F]+)(?:_d[0-9]+[pm][0-9a-fA-F]+)*))?",
        mode,
    )
    rules = {
    ...
    ordinal_dst_indexes = {}
    ordinal_dst_names = {}
    nlist_value_equalities = []
    nlist_name_equalities = []
    nlist_entry_equalities = []
    nlist_value_deltas = []
    address_dup_map = {}
    address_swap_pair = None
    address_dup_any = None
    address_dup_opcode = None
    address_patch_adrp = None
    address_patch_after = None
    address_patch_pair_regs = None
    target_word_patch = None
    if single_match:
        ...
        if mapval_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[mapval_match.group(1)]
        dst_name = src_name
        scope = "ordinal_map"
        for part in mapval_match.group(2).split("_"):
            source_text, dest_text = part.split("to", 1)
            if dest_text not in names_by_ordinal:
                lines.append(f"unknown CPI destination ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
            ...
            dest_name = names_by_ordinal[dest_text]
            ordinal_dst_names[source_ordinal] = dest_name
        for part in mapval_match.group(3).split("_"):
            left_text, right_text = part[1:].split("eq", 1)
            if left_text not in names_by_ordinal or right_text not in names_by_ordinal:
                lines.append(f"unknown CPI value ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 2
            nlist_value_equalities.append((names_by_ordinal[left_text], names_by_ordinal[right_text]))
    elif mapname_match:
        if mapname_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[mapname_match.group(1)]
        dst_name = src_name
        scope = "ordinal_map"
        for part in mapname_match.group(2).split("_"):
            source_text, dest_text = part.split("to", 1)
            if dest_text not in names_by_ordinal:
                lines.append(f"unknown CPI destination ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
            ...
            dest_name = names_by_ordinal[dest_text]
            ordinal_dst_names[source_ordinal] = dest_name
        for part in mapname_match.group(3).split("_"):
            left_text, right_text = part[1:].split("eq", 1)
            if left_text not in names_by_ordinal or right_text not in names_by_ordinal:
                lines.append(f"unknown CPI name ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 2
            nlist_name_equalities.append((names_by_ordinal[left_text], names_by_ordinal[right_text]))
    elif mapentry_match:
        if mapentry_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[mapentry_match.group(1)]
        dst_name = src_name
        scope = "ordinal_map"
        for part in mapentry_match.group(2).split("_"):
            source_text, dest_text = part.split("to", 1)
            if dest_text not in names_by_ordinal:
                lines.append(f"unknown CPI destination ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
            ...
            dest_name = names_by_ordinal[dest_text]
            ordinal_dst_names[source_ordinal] = dest_name
        for part in mapentry_match.group(3).split("_"):
            left_text, right_text = part[1:].split("eq", 1)
            if left_text not in names_by_ordinal or right_text not in names_by_ordinal:
                lines.append(f"unknown CPI entry ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 2
            nlist_entry_equalities.append((names_by_ordinal[left_text], names_by_ordinal[right_text]))
    elif mapdelta_match:
        if mapdelta_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[mapdelta_match.group(1)]
        dst_name = src_name
        scope = "ordinal_map"
        for part in mapdelta_match.group(2).split("_"):
            source_text, dest_text = part.split("to", 1)
            if dest_text not in names_by_ordinal:
                lines.append(f"unknown CPI destination ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
            ...
            dest_name = names_by_ordinal[dest_text]
            ordinal_dst_names[source_ordinal] = dest_name
        for part in mapdelta_match.group(3).split("_"):
            m = re.fullmatch(r"d([0-9]+)([pm])([0-9a-fA-F]+)", part)
            if m is None or m.group(1) not in names_by_ordinal:
                lines.append(f"unknown CPI delta ordinal in mode: {mode}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 2
            delta = int(m.group(3), 16)
            if m.group(2) == "m":
                delta = -delta
            nlist_value_deltas.append((names_by_ordinal[m.group(1)], delta))
    elif addrdup_match:
    ...
    elif addrdupany_match:
        if addrdupany_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[addrdupany_match.group(1)]
        dst_name = src_name
        scope = "address_edit"
        address_dup_any = {
    ...
    elif addrpatchadrp_match:
        if addrpatchadrp_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[addrpatchadrp_match.group(1)]
        dst_name = src_name
        scope = "address_edit"
        address_patch_adrp = {
        ...
        if addrpatchadrp_match.group(4):
            for part in addrpatchadrp_match.group(4).split("_"):
                m = re.fullmatch(r"d([0-9]+)([pm])([0-9a-fA-F]+)", part)
                if m is None or m.group(1) not in names_by_ordinal:
                    lines.append(f"unknown CPI delta ordinal in mode: {mode}")
                    Path(log_path).write_text("\n".join(lines) + "\n")
    ...
    elif addrpatchpairregs_match:
        if addrpatchpairregs_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[addrpatchpairregs_match.group(1)]
        dst_name = src_name
        scope = "address_edit"
        address_patch_pair_regs = {
            "adrp_ordinal": int(addrpatchpairregs_match.group(2)),
            "after_ordinal": int(addrpatchpairregs_match.group(3)),
            "delta": int(addrpatchpairregs_match.group(4), 16),
            "adrp_word": int(addrpatchpairregs_match.group(5), 16),
            "ldr_word": int(addrpatchpairregs_match.group(6), 16),
        }
        if addrpatchpairregs_match.group(7):
            for part in addrpatchpairregs_match.group(7).split("_"):
                m = re.fullmatch(r"d([0-9]+)([pm])([0-9a-fA-F]+)", part)
                if m is None or m.group(1) not in names_by_ordinal:
                    lines.append(f"unknown CPI delta ordinal in mode: {mode}")
                    Path(log_path).write_text("\n".join(lines) + "\n")
                ...
                if m.group(2) == "m":
                    delta = -delta
                nlist_value_deltas.append((names_by_ordinal[m.group(1)], delta))
    elif targetword_match:
        if targetword_match.group(1) not in names_by_ordinal:
            lines.append(f"unknown CPI ordinal in mode: {mode}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        src_name = names_by_ordinal[targetword_match.group(1)]
        dst_name = src_name
        scope = "address_edit"
        target_word_patch = {"word": int(targetword_match.group(2), 16)}
        if targetword_match.group(3):
            for part in targetword_match.group(3).split("_"):
                m = re.fullmatch(r"d([0-9]+)([pm])([0-9a-fA-F]+)", part)
                if m is None or m.group(1) not in names_by_ordinal:
                    lines.append(f"unknown CPI delta ordinal in mode: {mode}")
                    Path(log_path).write_text("\n".join(lines) + "\n")
        ...
        Path(log_path).write_text("\n".join(lines) + "\n")
        return 2
    for left_name, right_name in nlist_value_equalities:
        left = found[left_name]
        right = found[right_name]
        struct.pack_into("<Q", data, left["entry_off"] + 8, right["value"])
        lines.append(
            f"set_nlist_value {left_name}: 0x{left['value']:x} -> "
            f"0x{right['value']:x} from {right_name}"
        )
    for name, delta in nlist_value_deltas:
        info = found[name]
        new_value = info["value"] + delta
        if new_value < 0 or new_value > 0xffffffffffffffff:
            lines.append(f"nlist value delta out of range for {name}: {delta:+#x}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        struct.pack_into("<Q", data, info["entry_off"] + 8, new_value)
        lines.append(
            f"add_nlist_value {name}: 0x{info['value']:x} {delta:+#x} -> "
            f"0x{new_value:x}"
        )
    if nlist_value_equalities or nlist_value_deltas:
        found = nlist_symbols_by_name(data, symbols)
    symoff, nsyms, stroff, strsize = macho_symtab(data)
    for left_name, right_name in nlist_name_equalities:
        left = found[left_name]
        right = found[right_name]
        left_name_off = stroff + left["strx"]
        right_name_off = stroff + right["strx"]
        left_end = data.find(b"\0", left_name_off, min(stroff + strsize, len(data)))
        right_end = data.find(b"\0", right_name_off, min(stroff + strsize, len(data)))
        if left_end < 0 or right_end < 0:
            lines.append(f"missing string terminator for {left_name} or {right_name}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        old = bytes(data[left_name_off:left_end])
        new = bytes(data[right_name_off:right_end])
        if len(old) != len(new):
            lines.append(f"refusing non-same-length name equality {left_name} <- {right_name}")
            Path(log_path).write_text("\n".join(lines) + "\n")
            return 2
        data[left_name_off:left_end] = new
        lines.append(
            f"set_nlist_name {left_name}: {old.decode('ascii', 'replace')} -> "
            f"{new.decode('ascii', 'replace')} from {right_name}"
        )
    for left_name, right_name in nlist_entry_equalities:
        left = found[left_name]
        right = found[right_name]
        data[left["entry_off"]:left["entry_off"] + 16] = right["entry"]
        lines.append(
            f"set_nlist_entry {left_name}: index={left['index']} "
            f"<- {right_name}: index={right['index']} "
            f"value=0x{right['value']:x} strx={right['strx']}"
        )
    ...
    if address_patch_adrp is not None:
        lines.append(
            f"address_patch_adrp target_ordinal={address_patch_adrp['target_ordinal']} "
            f"delta=0x{address_patch_adrp['delta']:x}"
        )
    if address_patch_after is not None:
        lines.append(
            f"address_patch_after target_ordinal={address_patch_after['target_ordinal']} "
            f"delta=0x{address_patch_after['delta']:x}"
        )
    if target_word_patch is not None:
        lines.append(f"target_word_patch word=0x{target_word_patch['word']:08x}")
    lines.append(
    ...
    if scope == "address_edit":
        ...
        if address_patch_adrp is not None:
            needed.add(address_patch_adrp["target_ordinal"])
        if address_patch_after is not None:
            needed.add(address_patch_after["target_ordinal"])
        if address_patch_pair_regs is not None:
            needed.add(address_patch_pair_regs["adrp_ordinal"])
            needed.add(address_patch_pair_regs["after_ordinal"])
        ...
        if address_dup_any is not None:
            if target_addr is None:
                lines.append("addrdupany requires target_addr")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            target_ordinal = address_dup_any["target_ordinal"]
            min_delta = address_dup_any["min_delta"]
            target_entry = source_entries[target_ordinal]
            target_bits = target_entry["bits"]
            candidates = []
            for i in range(text["nreloc"]):
                rel_off = text["reloff"] + i * 8
                if rel_off + 8 > len(data):
                ...
                if not bits["extern"]:
                    continue
                if bits["type"] != target_bits["type"]:
                    continue
                if bits["pcrel"] != target_bits["pcrel"] or bits["length"] != target_bits["length"]:
                    continue
                rel_addr = text["addr"] + r_address_u
                delta = target_addr - rel_addr
                if delta <= 0 or delta < min_delta:
                    continue
                candidates.append((delta, i, rel_off, r_address_u, r_word, bits))
            candidates.sort(key=lambda item: (item[0], item[1]))
            if not candidates:
                lines.append(
                    f"addrdupany no candidates for target_ordinal={target_ordinal} "
                    f"min_delta=0x{min_delta:x}"
                )
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            delta, rel_index, rel_off, source_address, source_word, source_bits = candidates[0]
            struct.pack_into("<I", data, target_entry["rel_off"], source_address)
            changed += 1
            lines.append(
                f"dup_any_address ordinal={target_ordinal} rel[{target_entry['rel_index']}] "
                f"0x{target_entry['address']:x}->0x{source_address:x} "
                f"from rel[{rel_index}] delta_to_target=0x{delta:x} "
                f"symbolnum={source_bits['symbolnum']} type={source_bits['type']} "
                f"pcrel={source_bits['pcrel']} len={source_bits['length']} "
                f"word=0x{source_word:08x}"
            )
            for cand_delta, cand_index, _, cand_address, cand_word, cand_bits in candidates[:12]:
                lines.append(
                    f"dup_any_candidate rel[{cand_index}] section_off=0x{cand_address:x} "
        ...
        if target_word_patch is not None:
            if target_addr is None:
                lines.append("targetword requires target_addr")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            target_section_off = target_addr - text["addr"]
            if target_section_off < 0 or target_section_off + 4 > text["size"]:
                lines.append(
                    f"targetword out of text target_addr=0x{target_addr:x} "
                    f"section_off=0x{target_section_off:x}"
                )
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            if target_section_off % 4:
                lines.append(f"targetword unaligned section_off=0x{target_section_off:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            target_word_off = text["offset"] + target_section_off
            old_word, = struct.unpack_from("<I", data, target_word_off)
            new_word = target_word_patch["word"]
            struct.pack_into("<I", data, target_word_off, new_word)
            changed += 1
            lines.append(
                f"patch_target_word section_off=0x{target_section_off:x} "
                f"old_word=0x{old_word:08x} new_word=0x{new_word:08x}"
            )
        if address_patch_pair_regs is not None:
            if target_addr is None:
                lines.append("addrpatchpairregs requires target_addr")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            adrp_ordinal = address_patch_pair_regs["adrp_ordinal"]
            after_ordinal = address_patch_pair_regs["after_ordinal"]
            delta = address_patch_pair_regs["delta"]
            adrp_word = address_patch_pair_regs["adrp_word"]
            ldr_word = address_patch_pair_regs["ldr_word"]
            adrp_entry = source_entries[adrp_ordinal]
            after_entry = source_entries[after_ordinal]

            adrp_word_off = text["offset"] + adrp_entry["address"]
            if adrp_word_off + 4 > len(data):
                lines.append(f"ADRP pair word write out of range at 0x{adrp_word_off:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            old_adrp_word, = struct.unpack_from("<I", data, adrp_word_off)

            source_addr = target_addr + delta
            if source_addr < text["addr"] or source_addr + 4 > text["addr"] + text["size"]:
                lines.append(
                    f"addrpatchpairregs after source out of text target_addr=0x{target_addr:x} "
                    f"delta=0x{delta:x} source_addr=0x{source_addr:x}"
                )
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            source_address = source_addr - text["addr"]
            if source_address % 4:
                lines.append(f"addrpatchpairregs unaligned after source section_off=0x{source_address:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            ldr_word_off = text["offset"] + source_address
            old_ldr_word, = struct.unpack_from("<I", data, ldr_word_off)
            rel_addresses = {
                struct.unpack_from("<I", data, text["reloff"] + i * 8)[0]
                for i in range(text["nreloc"])
                if text["reloff"] + i * 8 + 4 <= len(data)
            ...
            struct.pack_into("<I", data, adrp_word_off, adrp_word)
            struct.pack_into("<I", data, ldr_word_off, ldr_word)
            struct.pack_into("<I", data, after_entry["rel_off"], source_address)
            changed += 3
            lines.append(
                f"patch_pair_regs adrp_ordinal={adrp_ordinal} rel[{adrp_entry['rel_index']}] "
                f"addr=0x{adrp_entry['address']:x} old_adrp_word=0x{old_adrp_word:08x} "
                f"new_adrp_word=0x{adrp_word:08x} "
                f"after_ordinal={after_ordinal} rel[{after_entry['rel_index']}] "
                f"0x{after_entry['address']:x}->0x{source_address:x} "
                f"delta_from_target=0x{delta:x} old_ldr_word=0x{old_ldr_word:08x} "
                f"new_ldr_word=0x{ldr_word:08x} "
                f"source_had_text_reloc={1 if source_address in rel_addresses else 0}"
            )
        if address_patch_adrp is not None:
            if target_addr is None:
                lines.append("addrpatchadrp requires target_addr")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            target_ordinal = address_patch_adrp["target_ordinal"]
            delta = address_patch_adrp["delta"]
            target_entry = source_entries[target_ordinal]
            original_word_off = text["offset"] + target_entry["address"]
            if original_word_off + 4 > len(data):
                lines.append(f"original ADRP read out of range at 0x{original_word_off:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            ...
            if source_addr < text["addr"] or source_addr + 4 > text["addr"] + text["size"]:
                lines.append(
                    f"addrpatchadrp source out of text target_addr=0x{target_addr:x} "
                    f"delta=0x{delta:x} source_addr=0x{source_addr:x}"
                )
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            source_address = source_addr - text["addr"]
            if source_address % 4:
                lines.append(f"addrpatchadrp unaligned source section_off=0x{source_address:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            source_word_off = text["offset"] + source_address
            old_word, = struct.unpack_from("<I", data, source_word_off)
            rel_addresses = {
                struct.unpack_from("<I", data, text["reloff"] + i * 8)[0]
                for i in range(text["nreloc"])
                if text["reloff"] + i * 8 + 4 <= len(data)
            ...
            changed += 2
            lines.append(
                f"patch_adrp_address ordinal={target_ordinal} rel[{target_entry['rel_index']}] "
                f"0x{target_entry['address']:x}->0x{source_address:x} "
                f"delta_to_target=0x{delta:x} old_word=0x{old_word:08x} "
                f"new_word=0x{original_word:08x} "
                f"source_had_text_reloc={1 if source_address in rel_addresses else 0}"
            )
        if address_patch_after is not None:
            if target_addr is None:
                lines.append("addrpatchafter requires target_addr")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            target_ordinal = address_patch_after["target_ordinal"]
            delta = address_patch_after["delta"]
            target_entry = source_entries[target_ordinal]
            original_word_off = text["offset"] + target_entry["address"]
            if original_word_off + 4 > len(data):
                lines.append(f"original after-target word read out of range at 0x{original_word_off:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            original_word, = struct.unpack_from("<I", data, original_word_off)
            ...
            if source_addr < text["addr"] or source_addr + 4 > text["addr"] + text["size"]:
                lines.append(
                    f"addrpatchafter source out of text target_addr=0x{target_addr:x} "
                    f"delta=0x{delta:x} source_addr=0x{source_addr:x}"
                )
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            source_address = source_addr - text["addr"]
            if source_address % 4:
                lines.append(f"addrpatchafter unaligned source section_off=0x{source_address:x}")
                Path(log_path).write_text("\n".join(lines) + "\n")
                return 44
            source_word_off = text["offset"] + source_address
            old_word, = struct.unpack_from("<I", data, source_word_off)
...
def cpi_reloc_transforms(prefix, symbols, mode):
    if mode not in ("cpi01_relocs", "cpi01_reloc_singles", "cpi01_reloc_pairs", "cpi01_reloc_pair_sweep", "cpi01_reloc_pair_mixed", "cpi01_reloc_pair_value", "cpi01_reloc_pair_name", "cpi01_reloc_pair_entry", "cpi01_reloc_pair_diag_meta", "cpi01_reloc_pair_value_delta", "cpi01_reloc_pair_value_grid", "cpi01_reloc_pair_value_grid_hi", "cpi01_reloc_pair_value_grid_hi2", "cpi01_reloc_pair_value_grid_neg", "cpi01_reloc_pair_value_grid_neg2", "cpi01_reloc_pair_value_grid_symbols", "cpi01_reloc_addr", "cpi01_reloc_addr_window", "cpi01_reloc_addr_any_window", "cpi01_reloc_addr_opcode_window", "cpi01_reloc_addr_patch_adrp_window", "cpi01_reloc_addr_patch_adrp_fine", "cpi01_reloc_addr_patch_adrp_value", "cpi01_reloc_addr_patch_after_window", "cpi01_reloc_addr_patch_after_fine", "cpi01_reloc_addr_patch_after_exact", "cpi01_reloc_addr_patch_after_ldr_regs", "cpi01_reloc_addr_patch_pair_regs", "cpi01_reloc_addr_patch_pair_opcodes", "cpi01_reloc_addr_patch_pair_opcode_value", "cpi01_reloc_addr_patch_pair_imm_value", "cpi01_reloc_addr_patch_pair_rt_value", "cpi01_reloc_addr_patch_pair_adrp_imm_value", "cpi01_reloc_addr_patch_pair_reg_value", "cpi01_reloc_addr_patch_pair_after_value", "cpi01_reloc_target_word_rd_value", "cpi01_reloc_target_word_payload_value", "cpi01_reloc_target_word_opcode_value", "cpi01_reloc_target_word_bit21_value", "cpi01_reloc_pageoff_compare", "cpi01_reloc_pageoff_compare_value_grid") or len(symbols) < 3:
        return []
    if mode == "cpi01_reloc_pageoff_compare_value_grid":
        bucket_deltas = [
            None,
            "d1m0800",
            "d1m0200",
    ...
    if mode == "cpi01_reloc_pageoff_compare":
        modes = [
            "targetword_1_waa0303fc",
            "targetword_1_waa0303fc_d1p0040",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    ...
    if mode == "cpi01_reloc_target_word_opcode_value":
        target_words = [
            ...
            "aa0303fc",  # ORR x28, xzr, x3
            "ca0303fc",  # EOR x28, xzr, x3
            "ea0303fc",  # ANDS x28, xzr, x3
            "8b0303fc",  # ADD x28, xzr/sp, x3
            "cb0303fc",  # SUB x28, xzr/sp, x3
            "d503201f",  # NOP
            "d65f03c0",  # RET
        ]
        bucket_deltas = [
        ...
        for target_word in target_words:
            ...
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_target_word_payload_value":
        target_words = [
            "aa0003fc",  # ORR x28, xzr, x0
            "aa0103fc",  # ORR x28, xzr, x1
            "aa0303fc",  # ORR x28, xzr, x3
            "aa0503fc",  # ORR x28, xzr, x5
            "aa1f03fc",  # ORR x28, xzr, x31
            "aa0307fc",  # ORR x28, xzr, x3, lsl #1
            "aa0317fc",  # ORR x28, xzr, x3, lsl #5
            "aa037ffc",  # ORR x28, xzr, x3, lsl #31
        ]
        bucket_deltas = [
            None,
            "d1m0800",
        ...
        for target_word in target_words:
            ...
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_target_word_rd_value":
        target_words = [
            "aa0303e0",  # ORR x0, xzr, x3
            "aa0303e1",  # ORR x1, xzr, x3
            "aa0303e5",  # ORR x5, xzr, x3
            "aa0303fc",  # ORR x28, xzr, x3
            "aa0303ff",  # ORR x31, xzr, x3
        ]
        bucket_deltas = [
            None,
            "d1m0800",
    ...
    if mode == "cpi01_reloc_pair_value_grid_symbols":
        modes = []
        for dest in (0, 1, 2):
            modes.append(f"map_1_3to{dest}_4to{dest}")
            for bucket in range(-4, 10):
                if bucket == 0:
                    continue
                delta = bucket * 0x10
                sign = "p" if delta > 0 else "m"
                modes.append(f"mapdelta_1_3to{dest}_4to{dest}_d{dest}{sign}{abs(delta):04x}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_value_grid_hi":
        modes = [
    ...
    if mode == "cpi01_reloc_pair_value_grid_hi2":
        ...
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_value_grid_neg":
        modes = [
            f"mapdelta_1_3to1_4to1_d1m{abs(bucket * 0x10):04x}"
            for bucket in range(-64, -8)
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_value_grid_neg2":
        modes = [
            f"mapdelta_1_3to1_4to1_d1m{abs(bucket * 0x10):04x}"
            for bucket in range(-128, -64)
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_value_grid":
        modes = ["map_1_3to1_4to1"]
        for bucket in range(-8, 25):
            if bucket == 0:
                continue
            delta = bucket * 0x10
            sign = "p" if delta > 0 else "m"
            modes.append(f"mapdelta_1_3to1_4to1_d1{sign}{abs(delta):04x}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_value_delta":
        deltas = [
            "m0040",
            "m0030",
            "m0020",
        ...
        modes = ["map_1_3to1_4to1"] + [
            f"mapdelta_1_3to1_4to1_d1{delta}" for delta in deltas
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_diag_meta":
        modes = [
            "map_1_3to0_4to0",
            "mapval_1_3to0_4to0_v0eq1",
            "mapentry_1_3to0_4to0_e0eq1",
            "mapval_1_3to0_4to0_v0eq2",
            "mapentry_1_3to0_4to0_e0eq2",
            "map_1_3to1_4to1",
            "mapval_1_3to1_4to1_v1eq0",
            "mapentry_1_3to1_4to1_e1eq0",
            "mapval_1_3to1_4to1_v1eq2",
            "mapentry_1_3to1_4to1_e1eq2",
            "map_1_3to2_4to2",
            "mapval_1_3to2_4to2_v2eq0",
            "mapentry_1_3to2_4to2_e2eq0",
            "mapval_1_3to2_4to2_v2eq1",
            "mapentry_1_3to2_4to2_e2eq1",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_entry":
        modes = [
            "map_1_3to0_4to1",
            "mapentry_1_3to0_4to1_e0eq1",
            "mapentry_1_3to0_4to1_e1eq0",
            "map_1_3to2_4to1",
            "mapentry_1_3to2_4to1_e2eq1",
            "map_1_3to0_4to2",
            "mapentry_1_3to0_4to2_e0eq2",
            "mapentry_1_3to0_4to2_e0eq1_e2eq1",
            "mapentry_1_3to1_4to2_e2eq1",
            "mapentry_1_3to2_4to0_e2eq0",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_name":
        modes = [
            "map_1_3to0_4to1",
            "mapname_1_3to0_4to1_n0eq1",
            "mapname_1_3to0_4to1_n1eq0",
            "map_1_3to2_4to1",
            "mapname_1_3to2_4to1_n2eq1",
            "map_1_3to0_4to2",
            "mapname_1_3to0_4to2_n0eq2",
            "mapname_1_3to0_4to2_n0eq1_n2eq1",
            "mapname_1_3to1_4to2_n2eq1",
            "mapname_1_3to2_4to0_n2eq0",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_pair_value":
        modes = [
            "map_1_3to0_4to1",
            "mapval_1_3to0_4to1_v0eq1",
            "map_1_3to2_4to1",
            "mapval_1_3to2_4to1_v2eq1",
            "map_1_3to0_4to2",
            "mapval_1_3to0_4to2_v0eq1_v2eq1",
            "mapval_1_3to0_4to2_v0eq2",
            "mapval_1_3to1_4to2_v2eq1",
            "mapval_1_3to2_4to0_v2eq0",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_pair_opcode_value":
        opcode_words = [
            "3dc000b8",  # LDR q24, [x5]
            "fd4000b8",  # LDR d24, [x5]
            "bd4000b8",  # LDR s24, [x5]
            "910000b8",  # ADD x24, x5, #0
        ]
        bucket_deltas = [
            None,
            "d1m0800",
        ...
        for ldr_word in opcode_words:
            ...
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_pair_imm_value":
        ldr_q_words = [
            "3dc000b8",  # LDR q24, [x5, #0]
            "3dc004b8",  # LDR q24, [x5, #16]
            "3dc010b8",  # LDR q24, [x5, #64]
            "3dc074b8",  # LDR q24, [x5, #464]
            "3dc100b8",  # LDR q24, [x5, #1024]
        ]
        bucket_deltas = [
            None,
            "d1m0800",
        ...
        modes = []
        for ldr_word in ldr_q_words:
            base = f"addrpatchpairregs_1_adrp4_after3_at0400_aw90000005_lw{ldr_word}"
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_pair_rt_value":
        ldr_q_words = [
            "3dc000a0",  # LDR q0, [x5]
            "3dc000b7",  # LDR q23, [x5]
            "3dc000b8",  # LDR q24, [x5]
        ...
        for ldr_word in ldr_q_words:
            ...
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_pair_adrp_imm_value":
        adrp_words = [
            "90000005",  # ADRP x5, imm21=0
            "b0000005",  # ADRP x5, imm21=1
            "90000025",  # ADRP x5, imm21=4
            "b00000e5",  # ADRP x5, imm21=29
            "90000205",  # ADRP x5, imm21=64
        ]
        bucket_deltas = [
            None,
            "d1m0800",
        ...
        modes = []
        for adrp_word in adrp_words:
            base = f"addrpatchpairregs_1_adrp4_after3_at0400_aw{adrp_word}_lw3dc000b8"
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_pair_reg_value":
        ...
        for adrp_word, ldr_word in reg_pairs:
            ...
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_pair_after_value":
        after_deltas = ["0400", "0800", "0c00", "1000", "1400"]
        bucket_deltas = [
            None,
            "d1m0800",
            "d1m0200",
        ...
        modes = []
        for after_delta in after_deltas:
            base = f"addrpatchpairregs_1_adrp4_after3_at{after_delta}_aw90000005_lw3dc000b8"
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    ...
    if mode == "cpi01_reloc_addr_patch_pair_regs":
        modes = [
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000005_lw3dc000b8",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000004_lw3dc00098",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000006_lw3dc000d8",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw9000001f_lw3dc003f8",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000004_lw3dc000b8",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000005_lw3dc00098",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000006_lw3dc000b8",
            "addrpatchpairregs_1_adrp4_after3_at0400_aw90000005_lw3dc000d8",
            "addrpatchpairregs_1_adrp4_after3_at0800_aw90000005_lw3dc000b8",
            "addrpatchpairregs_1_adrp4_after3_at0800_aw90000004_lw3dc00098",
            "addrpatchpairregs_1_adrp4_after3_at0800_aw90000006_lw3dc000d8",
            "addrpatchpairregs_1_adrp4_after3_at0800_aw90000004_lw3dc000b8",
            "addrpatchpairregs_1_adrp4_after3_at0800_aw90000005_lw3dc00098",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    ...
    if mode == "cpi01_reloc_addr_patch_after_fine":
        modes = [
            ...
            "addrpatchafter_1_3_at0340",
            "addrpatchafter_1_3_at0380",
            "addrpatchafter_1_3_at03c0",
            "addrpatchafter_1_3_at0400",
            "addrpatchafter_1_3_at0440",
            "addrpatchafter_1_3_at0480",
            "addrpatchafter_1_3_at04c0",
            "addrpatchafter_1_3_at0500",
            "addrpatchafter_1_3_at0b00",
            "addrpatchafter_1_3_at0b80",
            "addrpatchafter_1_3_at0bc0",
            "addrpatchafter_1_3_at0c00",
            "addrpatchafter_1_3_at0c40",
            "addrpatchafter_1_3_at0c80",
            "addrpatchafter_1_3_at0d00",
            "addrpatchafter_1_3_at1000",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_after_window":
        modes = [
            "addrpatchafter_1_3_at0400",
            "addrpatchafter_1_3_at0600",
            "addrpatchafter_1_3_at0700",
            "addrpatchafter_1_3_at0780",
            "addrpatchafter_1_3_at07c0",
            "addrpatchafter_1_3_at0800",
            "addrpatchafter_1_3_at0840",
            "addrpatchafter_1_3_at0880",
            "addrpatchafter_1_3_at0900",
            "addrpatchafter_1_3_at0a00",
            "addrpatchafter_1_3_at0c00",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_adrp_fine":
        modes = [
            "addrpatchadrp_1_4_at0300",
            "addrpatchadrp_1_4_at0340",
            "addrpatchadrp_1_4_at0380",
            "addrpatchadrp_1_4_at03c0",
            "addrpatchadrp_1_4_at0400",
            "addrpatchadrp_1_4_at0440",
            "addrpatchadrp_1_4_at0480",
            "addrpatchadrp_1_4_at04c0",
            "addrpatchadrp_1_4_at0500",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_adrp_value":
        adrp_deltas = ["0210", "0300", "0340", "0380", "03c0"]
        bucket_deltas = [
            None,
            "d1m0800",
            "d1m0200",
        ...
        modes = []
        for adrp_delta in adrp_deltas:
            base = f"addrpatchadrp_1_4_at{adrp_delta}"
            for delta in bucket_deltas:
                modes.append(base if delta is None else f"{base}_{delta}")
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_patch_adrp_window":
        modes = [
            "addrpatchadrp_1_4_at0210",
            "addrpatchadrp_1_4_at0300",
            "addrpatchadrp_1_4_at0400",
            "addrpatchadrp_1_4_at0500",
            "addrpatchadrp_1_4_at0600",
            "addrpatchadrp_1_4_at0800",
            "addrpatchadrp_1_4_at0c00",
            "addrpatchadrp_1_4_at1000",
            "addrpatchadrp_1_4_at1400",
            "addrpatchadrp_1_4_at1800",
            "addrpatchadrp_1_4_at1c00",
            "addrpatchadrp_1_4_at202c",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    ...
    if mode == "cpi01_reloc_addr_any_window":
        modes = [
            ...
            "addrdupany_1_4_ge0210",
            "addrdupany_1_4_ge0400",
            "addrdupany_1_4_ge0600",
            "addrdupany_1_4_ge0800",
            "addrdupany_1_4_ge0c00",
            "addrdupany_1_4_ge1000",
            "addrdupany_1_4_ge1400",
            "addrdupany_1_4_ge1800",
            "addrdupany_1_4_ge1c00",
            "addrdupany_1_4_ge2034",
        ]
        return [(f"{prefix}cpi01_reloc_{m}", symbols[:3], m) for m in modes]
    if mode == "cpi01_reloc_addr_window":
        modes = [
            "addrswap_1_4_7",
            "addrswap_1_4_8",
            "addrswap_1_4_9",
